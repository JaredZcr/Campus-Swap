...
async function request(path, options = {}) {
  ...
  // Backend returns { status_code: 1|0, msg, data }.
  // Normalize to { code: 200|0, msg, data } so UI checks keep working.
  if (data && typeof data.status_code !== 'undefined' && typeof data.code === 'undefined') {
    data.code = data.status_code === 1 ? 200 : 0;
  }

  // Backend paging wrapper is PageVo: { list: [...], count: n }
  // Some UI code expects `data.records` (AntD-style). Make both available.
...
export const findIdleByLabel = (idleLabel, page = 1, nums = 12) =>
  request(`/idle/lable?idleLabel=${idleLabel}&page=${page}&nums=${nums}`, { method: 'GET' });
...
// Convenience: pay is just an update with paymentStatus=1
...
export const uploadFile = async (file) => {
  ...
  const data = await res.json();
  // Normalize backend response: { status_code: 1|0, msg, data }
  // into the same shape as `request()` so UI checks keep working.
  if (data && typeof data.status_code !== 'undefined' && typeof data.code === 'undefined') {
    data.code = data.status_code === 1 ? 200 : 0;
  }
  return data;
...
// Label mapping (backend uses integers)
