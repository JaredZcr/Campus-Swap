...
const CONDITION_LABELS = ['Unknown', 'Like New', 'Good', 'Fair', 'Poor'];
...
function Listings({ category, searchQuery, priceRange, sortBy }) {
  ...
  useEffect(() => {
    ...
    const req = (label != null)
      ? findIdleByLabel(label, 1, 20)
      : findIdleItems(searchQuery, 1, 20);

    req.then((res) => {
      if (res.code === 200) {
        // Backend paging uses PageVo: { list: [...], count }, and api.js maps list -> records.
  ...
  let filtered = items.filter((item) => {
    ...
    const matchSearch = !searchQuery || item.idleName?.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  const toModalItem = (item) => ({
    ...
    condition: CONDITION_LABELS[item.idleLabel] || 'Good',
    seller: {
    ...
    category: Object.keys(LABEL_MAP).find((k) => LABEL_MAP[k] === item.idleLabel) || 'other',
              ...
              return (
                <div key={item.id} className="listing-card" onClick={() => setSelectedItem(toModalItem(item))}>
                  <div className="listing-image-container">
                    ...
                    />
                    <span className="condition-badge">{CONDITION_LABELS[item.idleLabel] || 'Item'}</span>
                  </div>
