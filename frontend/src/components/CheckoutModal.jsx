...
import { addOrder, addOrderAddress, addAddress, getAddress, updateOrder } from '../api';
...
export default function CheckoutModal({ item, onClose, onCreated }) {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      if (oaRes.code !== 200) {
        await updateOrder({ id: orderRes.data.id, orderStatus: 4 }).catch(() => null);
        setError(oaRes.msg || 'Failed to attach address. The order was rolled back.');
        setCreating(false);
        return;
      }
